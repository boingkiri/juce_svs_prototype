...
public:
    ...
    //===
    /* This function can be called to load a midi file so that is can be played*/
...
private:
    //======
    /* Send note off / controller off / sound off on all the MIDI channels */
    ...
    bool isPlayingMIDI = false;                 // Tells if the last audio buffer included some MIDI content to play
    ...
    double nextStartTime = -1.0f;       // The start time in seconds of the next audio buffer
